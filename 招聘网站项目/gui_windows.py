...
def window_mainloop():
    ...
    # 置标题栏于上
    mainlbl = wslabel(window, "招聘查询系统-Demo-V3.2.0", font=('华文中宋', 20), x=315, y=17)
    mainlbl.place()
    ...
    intro_btn.trigger(lambda event: box_intro(event))
    intro_btn.place()
    # 其三用于展示信息
    ...
    wordcloud_btn = wsbutton(window, text='词云图', font=("华文中宋", 14),
                             width=19, height=1, x=0, y=633, bg="#87cefb")
    wordcloud_btn.place()
    ...
    table_btn = wsbutton(window, text='职位列表', font=("华文中宋", 14),
                         width=19, height=1, x=238, y=633, bg="#98fb98")
    table_btn.place()
    ...
    job_analysis = wsbutton(window, text='热门城市薪资', font=("华文中宋", 14),
                            width=19, height=1, x=476, y=633, bg="#f0e68c")
    ...
    job_analysis.place()
    # ------------------------------------------------------------------
    job_intro = wsbutton(window, text='使用说明', font=("华文中宋", 14),
                         width=19, height=1, x=714, y=633, bg="#ff778a")
    job_intro.place()
    job_intro.trigger(lambda event: usage_intro(event, info_canvas,
                                                table_canvas, pic_canvas,
                                                ana_canvas))
