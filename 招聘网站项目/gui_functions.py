...
def pop_up_tips():
    root = tk.Tk()
    root.geometry("250x125+550+300")
    root.title("数据获取提示框")
    root.resizable(0, 0)
    lb = tk.Label(root, text="正在更新数据库...\n请耐心等待1分钟左右\n请勿操作电脑",
                  font=("华文中宋", 15), fg="red")
    lb.place(x=32, y=10)
    bt = wsbutton(root, "好的", width=3, x=110, y=90)
    bt.place()
    bt.trigger(lambda event: close(event, root))
    root.mainloop()


def close(event, root):
    root.destroy()


def update_database(event, canvas0, canvas1, canvas2, canvas3, job, city, root_path, total_page=20):
    #                                      画布                  职    城     路径       爬取页面数
    """
    ...
    try:
        _thread.start_new_thread(pop_up_tips, ())
...
def box_intro(event):
    ...
    intro.geometry("900x600+255+120")
    intro.title("箱线图简介")
...
def usage_intro(event, canvas0, canvas1, canvas2, canvas3):
    canvas1.canvas.place_forget()
    canvas2.canvas.place_forget()
    canvas3.canvas.place_forget()
    canvas0.place()
    path = "数据库/instruct.jpg"
    canvas0.show_image(path)
