...
def search_for_job(job, num, city='全国'):
    ...
    url = 'https://search.51job.com/list/' + city_value + \
        ',000000,0000,00,9,99,' + job + ',2,' + str(num) + '.html'
...
def get_needed_data(txt):
    ...
    lst = []
    long_dic = wc.bs_craw(
        txt, "script", {"type": "text/javascript"}, return_type="text")
    obj0 = re.compile(
        '{"type":"engine.*?"is_sp.*?",(?P<info>.*?),"adid":""}', re.S)
...
#     resp.encoding = 'gbk'
#     text = resp.text
#     target_demand = wc.bs_craw(
#         text, "div", attr={"class": "bmsg job_msg inbox"}, return_type='text')
...
def list_fulfilling(job_info_list, job_name=[], company_name=[], company_type=[],
    ...
    return job_name, work_city, company_name, company_type, company_size, company_ind, date
